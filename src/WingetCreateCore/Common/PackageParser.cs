    ...
    public static class PackageParser
    {
        ...
        public static bool ParsePackageAndUpdateInstallerNode(Installer installer, string filePath, string url, string archivePath = null)
        {
            ...
            if (!parseResult)
            {
                ...
                parseResult = ParseMsi(filePath, installer, null, newInstallers);
            }
        ...
        private static bool ParsePackageAndGenerateInstallerNodes(InstallerMetadata installerMetadata, Manifests manifests)
        {
            ...
            foreach (string path in installerPaths)
            {
                ...
                if (!parseResult)
                {
                    // We only want to parse the MSI if the exe/msix parsing fails.
                    // The msi library is more strict and will throw exceptions at FFI boundary, which are harder to catch and handle.
                    parseResult = ParseMsi(path, baseInstaller, manifests, newInstallers);
                }
        ...
        private static MachineType GetCompatibleMachineType(MachineType type)
        {
            return type switch
            {
                MachineType.Armv7 => MachineType.Arm,
                _ => type,
            };
        }
        ...
        private static CompatibilitySet GetCompatibilitySet(InstallerType type)
        {
            return type switch
            {
                ...
                InstallerType.Portable or InstallerType.Inno or InstallerType.Nullsoft or InstallerType.Exe or InstallerType.Burn => CompatibilitySet.Exe,
                InstallerType.Wix or InstallerType.Msi => CompatibilitySet.Msi,
                InstallerType.Msix or InstallerType.Appx => CompatibilitySet.Msix,
                _ => CompatibilitySet.None,
            };
        }
